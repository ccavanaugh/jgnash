...
Executable files are provided for Windows and UN*X users at the root of the installation directory. (These are `.exe` and `bash shell` files, respectively.)
...
* Windows
Simply double click on the jGnash.exe file.

* UN*X / MacOS  Start jGnash with the provided *jGnash* Bash script.  If jGnash fails to launch, check your file permissions and
...
This will run the Gradle tasks necessary to execute core tests and create the distribution file.  The distributable zip file will be produced at the root of the build directory called jGnash-_version_-bin.zip.
