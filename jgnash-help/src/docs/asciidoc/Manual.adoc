...
[[_reminders]]
== Reminders
If you find yourself recreating the same transaction on a regular basis, reminders may be created to automate the process.
...
The *Last Posted* column show the last date the Reminder was processed and the *Due* column shows the next due date.
The *Last Posted* column will be empty for a new Reminder.

An existing Reminder can be modified by selecting it and clicking the btn:[Modify] button.
The btn:[Execute Now] button will process the selected Reminder if it is due.
The btn:[Check Reminders] button looks for any pending Reminders and displays the dialog shown below.  If there are not
any pending Reminders, the dialog will not be displayed.

When jGnash is stated, pending Reminders will be shown automatically after a short period of time.

image::remindersPopupDialog.png[width="640"]

The Reminder dialog allows you to snooze for a selectable period of time by clicking the btn:[Remind Me Later] button
if you do not want to process the transactions at that time.  The btn:[Acknowledge Selected] button processes _only_
the transactions marked in the *Approve* column.
...
****
Be careful when creating Reminders near the end of the month with a Month frequency.  For example, what happens if you
select February 29th (Leap Day)?  Do you receive a paycheck every two weeks (26 pay periods) or twice a month (24 pay periods)?
